...
The dated record of how the order came to be what it is. `RANKING.md` holds the *current* argument
and points here; this file is append-only and is read when a placement is argued with, not on every
re-rank. The split follows `CONCURRENCY.md` → `CONCURRENCY-INCIDENTS.md`: the rule in force stays in
the file you read, the narrative that produced it moves to the file you open on purpose.

Sections below are in the order they were written. **Tickets named here are mostly closed** — read a
section for the argument it records, not for the state of the queue.
