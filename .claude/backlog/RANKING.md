...
**It is dominated by findings sweeps, not by a project plan.** Twenty-six of the
rows came off `FINDINGS.md` on 2026-08-25 in three batches and fifteen more on 2026-09-05, one ticket per root
cause, and together they occupy most of the queue; the live count is `./next`'s to report, not this
file's. Below them sit the project slices that held the
...
| **0115** | **Inserted 2026-09-08, directly above 0090.** Tier 1 — silently wrong output, which the tier table calls the worst case because nobody is counting the damage: `./next --drift` is the repo's only drift instrument, three files tell sessions it catches a row and its item disagreeing, and it prints `no drift` over exactly that. Same family as 0090 — a contract gap `0081`'s passes exposed and were forbidden from taking — and it beats 0090 on **blast radius** (tie-breaker 1): 0090's four gaps fire when a session claims, closes or hands off, while a blind `--drift` is consulted at every session's orientation, at `verify` Step 5's post-close check, and by `orchestrate`'s exit-code routing, which spends `1` on drift and stops. Below 0106 on **capture order** (tie-breaker 5), the two being equal on the four above it. Not promoted over the cost-reduction set: it does not move the token figure those rows target. | *2026-09-08*, this row |
| **0111, 0114** (then 0112, 0113 lower) | The 2026-09-07 findings sweep, four rows off a buffer the same day's retro filled. **0111 and 0114 go directly under 0078 and above 0075**, all four being the same subject — how `retro` finds its input and gets its output out. 0111 first on the **regret operator**: skipping it leaves the next workspace-root retro re-inventing which buffers to sweep, and its privacy NFR is the sharp end — one pass reaching both a company-tracked backlog and this public repo. 0114 next, above 0075 on **blast radius** (tie-breaker 1): 0075's stale-tree edit fires only when a checkout is behind, while 0114's half-release fires on *every* agent-run retro that touches a skill; it sits below 0111 only because a known workaround exists (`--yes`). **0112 goes directly under 0063**, which rewrites the same `CLAUDE.md` paragraph — no `blocked_by`, either order works, but whoever lands second edits a paragraph the first moved. **0113 sits low, above 0097**: a real instruction conflict, but inert until a pass touches three repos, which is rarer than anything above it. | *2026-09-07*, "The findings sweep of 2026-09-07" |
...
| **0093, 0094** | The same sweep's stage-skill fixes, at the head of that cluster on blast radius: 0093 is the before-and-after suite comparison every red-attribution and every mutation sweep runs, rediscovered by three sessions in a row; 0094 is the reason a released row carries, measured once at a full re-derivation of 0086's overlap. 0093 above 0094 because a release is rare and an attribution is not. | *2026-09-05*, "The findings sweep of 2026-09-05" |
...
| **0066, 0063, 0096, 0095, 0067, 0043** | Tier 2, real but not accruing much; 0067 is dormant until someone starts a rename and beats 0043 only on blast radius. **0096 and 0095 were inserted on 2026-09-05**, both below 0063 — which protects every prose guard — and both above 0067, which is dormant: 0096 is the two frontmatter path lists, wrong from birth and unchecked, and 0095 is the `set -e` clean-path trap that has already bitten two guards. | *batch 3*, "Where the six went" |
...
| **0100, 0101** | Tier 4, above the capability tickets on tie-breaker 4 — both `s` and certain against three `l`-shaped rows. 0100 gives the two findings markers a form and a count, which is the number 0060's decision will want; 0101 is the batch case `CONVENTIONS.md` leaves undefined, more likely to be hit now that 0078 routes a finding by what it is about. | *2026-09-05*, "The findings sweep of 2026-09-05" |
...
| **0097, 0103, 0099, 0102, 0098** | Tier 5, the bottom of the queue. 0097 leads them on tie-breaker 3: the residual method for thinking tokens is reconstructable today and its reader is the 2026-10-31 re-measurement. 0103 next, its Tier-2 duplication trigger already fired at three copies of `decomment`. Then 0099, whose stale counts mislead a re-rank; 0102, two of 0028's deferrals coming due; and 0098, two ambiguous citations. | *2026-09-05*, "The findings sweep of 2026-09-05" |
...
opened for this** — 0039 already specifies it: FR2 batches by *gate* (the topmost takeable row
plus every other takeable row sharing its `expects:` scope), not by an arbitrary count, so the
"small chunk" is scoped by file ownership rather than a number; FR11 has the supervisor claim no
row itself, and AC4 batches only rows that already share file scope into one session, which is the
conflict-minimisation ask; FR7 states the turn-budget-per-cycle that is the actual cost lever. The
sequence stands as written: 0038 → 0039 unblocks orchestrate, the rest of the backlog resumes
after.
...
- **A findings sweep of comparable size.** The 2026-09-05 sweep filed fifteen rows and left eleven
  entries in the buffer for `retro`; a further sweep
