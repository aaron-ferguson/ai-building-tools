...
- 2026-09-05 [AetherWorks/0119] **0086 says two different things about whether `qa_level: verify` survives, and it is at row 2 ready to build.** FR8 says the enum *"gains `review`"* — additive, `verify` stays. Its design note then writes the settled schema as `qa_level ∈ {review, unit, integration, e2e}` (:280), which omits `verify` and so reads as a rename. The two readings are far apart in cost: additive touches the template and one skill, while a rename sweeps **51 items in this backlog alone** carrying `qa_level: verify`, plus every other project's — which is exactly what `0067` exists to price, and `0086` already `relates:` to it. A `develop` session will have to pick one, and picking silently is how a cross-cutting rename starts without anyone deciding to do one. Worth noting that a rename has a real argument behind it — the level `verify` and the *stage* `verify` are the same word for different things — so this is not obviously a typo to correct in passing. Found from outside, while settling AetherWorks 0119 (whether `qa_level: none` is a level); that ticket now reads its valid set from the shipped template rather than naming today's values, so it survives either answer (pointer: `.claude/backlog/items/0086-add-a-light-qa-tier.md` FR8 and :280, item `0067`).
