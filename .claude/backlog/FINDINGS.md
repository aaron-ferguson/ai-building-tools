...
- 2026-09-25 — verify 0180 (token 3d48): **a develop-written clause table does not end the rounds on its own, because a row can pin half of a compound Given.** Round 5's table had `AC5 Given: belongs to an open ticket` and no row for the qualifier in front of it, "whose **introducing commit** belongs to". Deleting `introducing commit` from the whole proposal section of `skills/sprint/SKILL.md` leaves `tests/sprint.test.sh` at `320 passed, 0 failed, 1 skipped`. Verify may only report this as a table omission, so a sixth round exists for one phrase. Candidate for develop Step 5 / verify Step 3: a Given row is the whole Given clause, verbatim, and its guard asserts all of it, never the predicate half. Subject: `skills/develop/SKILL.md`, `skills/verify/SKILL.md`.
- 2026-09-25 — develop 0180 (token bb92): **the verbatim Given rule (develop Step 5, a3b1a3e) can force a prose edit on a round a verdict called "guard work only".** Two of 0180's Givens ("a waived red", "the answer repair first") were only paraphrased in the prose. A guard cannot assert words the section does not contain, so the rule was met only by rewording the prose to use the AC's own words (62b4b3d). Neither develop Step 5 nor verify Step 3 says whether that is expected, or whether a paraphrased Given is itself a verify FAIL. Rounds 3–5 each passed AC3 and AC4 on the paraphrase.
