...
- 2026-09-20 — **A stage's self-reported `session_id` is not the session it ran in, and the ledger silently books a phantom gate at USD 0.00 when they differ.** The develop gate was dispatched with a pre-assigned `--session-id 39c7c2e3-bf0a-4112-9c5e-da90275e59f9`; its outcome object returned `session_id: c2435e20-010c-4888-964d-3f9e8d57e7b9`, for which no transcript exists. `sprint-ledger.sh record` pins its harvest to both the dispatch event and the outcome, so it emitted two GATE lines for one gate — the real one at USD 15.21 and a phantom at USD 0.00 — and a reader summing them under-reports nothing but over-counts sessions. `skills/sprint/SKILL.md` Step 3 states the pre-assignment exists precisely so the transcript path is a dispatch-time fact, but nothing reconciles the two ids, and the schema invites the stage to supply one it cannot know. **This still needs a row**: either drop `session_id` from the outcome schema in favour of the dispatch event, or have the ledger refuse an outcome id with no transcript instead of harvesting it to zero. (`skills/sprint/SKILL.md` Step 3; `skills/sprint/outcome.schema.json`; `tools/sprint-ledger.sh`; sprint supervisor run-20260920T222013Z)
- 2026-09-20 — **The deferral NOTE claims a confirmed scope that was never confirmed.** With no `--scope` at all, `./next --drive` over a crossed buffer and one takeable row prints `NOTE      the findings gate crossed and is deferred until confirmed scope is finished`. The behaviour is FR4's and right — the gate waits for the `complete` site so a hand-driven run finishes its work first — but a supervisor reading that line on a run that confirmed no scope is told about a scope it does not have, and the obvious next move is to go looking for the `--scope` ids it thinks it passed. One sentence, two cases: *deferred until the run is out of work* where `SCOPE` is empty. (`skills/queue/templates/next` `findings_gate`; verify 0168, claim 8b1d)
- 2026-09-21 — **A sprint that spans two days is a normal use case, and four separate mechanisms in `sprint` assume one continuous run.** run-20260920T222013Z was interrupted three times — an account session limit resetting at 8:50pm, then the background wrapper's 600s ceiling, then the host sleeping mid-response twice — and resumed across a date boundary. What broke or would have broken: **(1) the cap formula.** Step 7 caps a resumed leg at "stage cap less what a harvest already shows spent", but every resume re-pays the session's context floor, so the remainder shrinks toward zero while the unfinished scope does not: at the third resume the formula gave USD 3.21 for two untouched tickets against a two-ticket price of 9.08, and obeying it would have fired mid-verify. The floor is a cost of *being interrupted*, not of scope, and nothing subtracts it. **(2) the supervisor marker.** `.active/held-by` is kept fresh by a heartbeat that lives only inside one Bash call, and `lock_stale_seconds` is 900s — so any overnight or multi-hour gap ages the marker out, and a supervisor resuming its own run next morning reads its own marker as a stale one to take over. **(3) `sprint_ended` and the findings gate.** One interrupted sprint resumed later is one sprint, but nothing says so: writing the event at each halt inflates `findings_max_sprints` (the age half of the gate) on a technicality, and writing it once means a run that never resumes is never counted. This run wrote it once, by hand, on the supervisor's judgment. **(4) the ledger.** `record` appends one block per invocation and derives wall-clock from the first and last event timestamps, so an overnight gap books as work time and a second invocation double-counts the run — 0164 covers the elapsed/active split but not the re-invocation. **This still needs a row**, and it is a design question before it is a defect: what identifies a sprint across a suspension, and which of the four mechanisms keys off that identity rather than off the process. (`skills/sprint/SKILL.md` Steps 1, 6, 7; `tools/sprint-ledger.sh`; related 0164; sprint supervisor run-20260920T222013Z)
