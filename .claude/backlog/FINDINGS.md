...
- 2026-09-12 (design) — **A second route to the same red: `design` Step 4's "say which criteria it confirms, which it changes, which it adds" invites a bulleted cross-reference list inside `## Acceptance criteria`, which `tests/item-ac-form.test.sh` reads as unticked non-checkbox criteria.** 0041's first design commit (`4e19572`) did exactly that. This time the guard was run by hand before the handoff, so the fix (`19649ec`) landed on design's own time, but only because the entry above was fresh. Step 4 could say to write the confirm/change/add accounting as prose, or in *Notes & decisions* (pointer: `skills/design/SKILL.md` Step 4 item 2; item 0041).
- 2026-09-12 (design) — **`expects:` paths go stale on a rename, and nothing notices.** Open row 0137 still expects `skills/orchestrate/SKILL.md`, which 0129 renamed to `skills/sprint/SKILL.md`, as did 0041 until this pass. `./next` prints `EXPECTS` and crosses it against held files, so a stale path can never collide with the file that is really held. `./next --drift` checks stage, status and claims, but not whether an expected path exists (pointer: `.claude/backlog/next` `--drift`; items 0137, 0129).
- 2026-09-12 (verify) — **A blocker whose NFR says "the dependent stays blocked until this is *released*" has no verify path: `./close` reconciles the dependent to ready on close, and verify Step 5 offers no branch for "green, but not yet shipped".** 0150 was green on every AC with the installed 0.9.25 still lacking the change, so closing would have unblocked 0134 against an install that cannot hold its row. Resolved here only by asking the user to release first; the alternative was `verify waiting` naming the release. `verify` Step 5 or `queue`'s NFR guidance could name one of those as the rule, or `blocked_by` could express "released" (pointer: `skills/verify/SKILL.md` Step 5; `.claude/backlog/close` reconcile; items 0150, 0134).
- 2026-09-12 (develop) — **`./next --drive --propose` describes nothing below an unheld design row, so a sprint whose scope includes one cannot show the person the develop gate it will dispatch next.** The rank walk escalates on the design row before a gate is formed, so there is no `PROPOSE` block and `DEPTH` gives only a count. 0134's skill text names the design row and relies on the partial-gate rule once `--drive` names the gate after the claim lands. The confirmed scope therefore holds the gate's ids only by way of `./next develop`'s lead row. A `--propose` that also described the gate beneath an in-scope design row would close it (pointer: `.claude/backlog/next` rank walk and `propose_block`; `skills/sprint/SKILL.md` *The proposal*; item 0134).
- 2026-09-13 (sprint) — **Every `/sprint` stage dispatch fails before a session starts: `claude -p` 2.1.270 rejects `outcome.schema.json` with *"--json-schema is not a valid JSON Schema: no schema with key or ref https://json-schema.org/draft/2020-12/schema"*.** Run `run-20260913T021045Z` dispatched `retro` for the findings tail; the CLI exited 1 with empty stdout, so the run escalated with nothing done. A scratchpad copy with only the top-level `$schema` key removed was accepted and returned a conforming object, so the key alone is the cause. The trivial Step 1 probe passes, because its inline schema carries no `$schema`, so the probe cannot catch this; the probe could use the real schema file (pointer: `skills/sprint/outcome.schema.json` line 2; `skills/sprint/SKILL.md` Step 1 and Step 3; `.claude/backlog/runs/run-20260913T021045Z.jsonl`).
- 2026-09-13 (sprint) — **`tools/sprint-ledger.sh estimate` prices a tail-only sprint at 0 tokens and USD 0.00.** With `--tickets 0 --develop-gates 0 --verify-sessions 0` (the `--drive` exit 5 case, where the whole run is `retro` then `queue`) every figure comes back zero and is labelled as derived from `MEASUREMENT.md`, while that table's per-session means put the tail near USD 6.75. The proposal then shows a derived-looking zero, and `record` would score the run against it. The estimator has no input for tail stages (pointer: `tools/sprint-ledger.sh` estimate; `skills/sprint/SKILL.md` *The proposal* and Step 6).
- 2026-09-13 (sprint) — **`config.yml` `stage_budget_usd` carries no `queue` cap, yet Step 6 dispatches `queue` as half of every tail.** Step 7 forbids the supervisor choosing a cap, so any run reaching the findings gate must stop and ask. On `run-20260913T021045Z` the user set 12.72 (the 4.24 per-session mean times 3), and raised `retro` from 3.75 to 11.25 because the configured base is a mean for an ordinary buffer and this one held 71 entries, about 9x the threshold. Neither figure is recorded in `config.yml`, and a cap that does not scale with buffer size will fire mid-sweep on exactly the runs that most need the tail (pointer: `.claude/backlog/config.yml` `stage_budget_usd`; `skills/sprint/SKILL.md` Step 7).
- 2026-09-13 (sprint) — **The supervisor marker's `held-by` pid is dead as soon as it is written.** Step 2 of `/sprint` says to record a pid and treat a marker whose pid is not alive as stale and take it over. A supervisor's Bash tool calls each run in a fresh shell, so `$$` names a process that has already exited, and a second supervisor following the rule would take over a live run. The supervisor has no long-lived pid of its own to record; staleness needs another signal (pointer: `skills/sprint/SKILL.md` Step 1 check 2; `.claude/backlog/runs/.active/held-by`).
- 2026-09-13 (sprint) — **`/sprint` has no step for parking findings, so what a run learns about its own tooling is lost when the supervising conversation ends.** Every other stage parks what surprised it and `retro` sweeps it back into the skills; `sprint` is a skill in the same toolkit and needs the same loop. The sections on what the skill never does and on not writing the backlog read as forbidding it, so four tooling defects found on `run-20260913T021045Z` reached only the report until the user asked for them to be recorded. `sprint` should park findings under the lock like any other writer, at the end of a run and whenever an escalation is about its own tooling rather than a stage's work (pointer: `skills/sprint/SKILL.md` Step 8 and Step 9; `references/CONCURRENCY.md` *Lock every write to the backlog directory*).
