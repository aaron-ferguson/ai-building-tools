...
- 2026-09-13 — **Under `set -eu`, a guard that captures `grep` into a variable without `|| true` aborts the test file on the exact input it exists to catch — no FAIL line, no tally.** A mutation run that greps for FAIL reads nothing, which looks neither red nor green. `tests/*.test.sh` share this shape; a guard should print its tally or exit loudly when it dies mid-file. (0153 develop eac85fd6; tests/sprint.test.sh)
- 2026-09-13 — **The live supervisor's own heartbeat is not the skill's snippet: it runs `sed` on an `at:` line of `held-by`, while `skills/sprint/SKILL.md` Step 3 writes `<run id> <UTC time>` with `printf`.** Step 1 reads the timestamp by the skill's format, so a marker kept fresh by the supervisor's form may not be readable as fresh by a second supervisor following the skill. (run-20260913T222409Z develop dispatch for 0153; skills/sprint/SKILL.md Steps 1 and 3)
- 2026-09-13 — verify (091e8968) on 0153: a prose-guard AC ("refreshes the signal") has now bounced three verify rounds, each naming one more grep shape to catch (M4 → M-target/M-notime → M-comment/M-nocall). A send-back constraint that dictates regex shapes invites the next uncaught shape. Where the guarded artifact is an executable snippet, verify should name execution of the snippet as the constraint on the first FAIL. Skill: verify Step 5, *Name the constraint*.
- 2026-09-14 — **The supervisor's hand-rolled marker heartbeat, stopped with `kill $BEAT`, left its `sleep 60` child orphaned under launchd.** Killing the backgrounded subshell does not kill the `sleep` it is waiting on; `pkill -P $BEAT` before `kill $BEAT` stopped it cleanly on later dispatches. 0153's Step 3 snippet should be checked for the same orphan, since the executed guard asserts the loop stops but not that no child survives. (run-20260913T222409Z verify 091e8968; skills/sprint/SKILL.md Step 3)
- 2026-09-14 — **The host killed a background verify stage for low memory (8 GB machine, 16% free), and `claude -p --resume` on the same session, still holding its claim, finished it with a valid PASS.** Step 7 names only a session-limit stop as a pause; a host kill that leaves a clean tree and a held claim recovers the same way, but it can recur while memory stays tight, and the kill notice arrives only through the background-task wrapper. (run-20260913T222409Z verify c8367270, stage_killed and resumed events; skills/sprint/SKILL.md Step 7)
