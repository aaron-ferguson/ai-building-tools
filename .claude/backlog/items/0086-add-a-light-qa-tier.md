...
### Re-verified 2026-09-09 by `3e7c` — the re-entry pass

`qa_level: unit` (frontmatter; the QA plan's `**Level:** unit` agrees — no drift). Suite run
file-by-file per `config.yml`'s attribution note: **24 files, 1205 assertions, 0 failed.** `lint`
and `typecheck` are unset in `config.yml`, so neither ran. Working tree clean at the level run and
at verdict.

**Which copy executed.** The `verify` skill driving this pass resolved from
`~/.claude/plugins/cache/ai-building-tools/ai-building-tools/0.9.19/`, and eight files under
`skills/` differ between that install and this checkout — including `skills/verify/SKILL.md`,
whose `review` level rows are part of what this ticket adds. The repo copy is the authority and is
what was checked; the installed copy is pre-0086 and will match at the next version bump.

Every AC was re-driven from scratch against independently scaffolded fixture repos, not read out of
`tests/close.test.sh`, and every mutation cited below was re-run rather than taken from the build
notes. **13 mutations plus one no-op control**; each confirmed landed by diff before its run, each
restored from a copy (never `git checkout --`), tree clean afterwards.

| Row | How it was checked | Result |
|---|---|---|
| AC1 | Fixture `0201` at `next: develop`, `close_by: develop`, both ACs citing a tracked `tests/thing.test.sh`. `close 0201 tok1` → exit 0, `closed 0201`, both ACs ticked, row gone from `QUEUE.md`, appended to `DONE.md`, `status: done`, `touches:`/`claimed_by:` cleared, tree clean | **PASS** |
| AC2 | Same row shape with `close_by: verify` (`0202`) and with the field absent (`0203`). Both exit 1 on the existing ground — *"is at stage 'develop', not 'verify'"* + *"verify owns closing"*; 0 ACs ticked, row intact, tree unchanged | **PASS** |
| AC3 | Fixture `0204`, AC2 reading *"verified by reading it carefully"*. Exit 1, names it by line and text: *"line 20: AC2 — second thing, verified by reading it carefully."* → *"names no committed path"*; tree unchanged. M2 (`describe_citation_problem` returns 0 unconditionally) → `close.test.sh` 16 failed | **PASS** |
| AC4 | Fixture `0205` at `qa_level: review` + `close_by: develop` → exit 1, *"those cannot hold together"*. M6 replaced the combined condition with `if false` → 4 failed | **PASS** |
| AC5 | **The 2026-09-08 red is fixed and re-driven.** Five shapes: `0206` 3 plain bullets → refused; **`0211` 3 `- [ ]` boxes with none ticked → refused, exit 1, *"lists 3 lines … and not one of them is ticked"*** (this exact fixture closed on exit 0 last pass); `0213` partly ticked → closes; `0214` all ticked → closes; `0215` empty section → closes. M1 reverted `rc_ticked` to the old `/^- \[[ xX]\]/` class → 8 failed, among them *"says nothing was TICKED, not that nothing is a checkbox"* | **PASS** |
| AC6 | This repo's `config.yml` has no `review:` block (0 matches — AC6/AC11's fixtures require that). `skills/verify/SKILL.md:155` carries *"`review` is not exempt: with no `review:` block configured, stop"* with its reason. M8 rewrote it to *"may be skipped where nothing is configured"* → `close-by.test.sh` 2 failed | **PASS** |
| AC7 | `develop` Step 5 reads `close_by` before choosing its ending (`:438`), states raise-only (`:449`), and carries the closing branch and its `CLOSED` last line (`:568`). M9 made the ending unconditional → 1 failed; M10 allowed either direction → 2 failed | **PASS** |
| AC8 | `tests/graph-fields.test.sh:128` enumerates `close_by`. `./next verify` printed the live take line this session: `TAKE 0086 \| … \| size l \| qa unit \| close verify` — printed unconditionally, and 0086 carries no `close_by:` line at all. M11 dropped the key from the template → `graph-fields` 1 failed; M12 removed it from `next`'s printf → `next.test.sh` 2 failed | **PASS** |
| AC9 | Fixture `0208`, no `close_by:` line anywhere, at `next: verify` → closes normally, 1 AC ticked, `status: done`. `claim` and `next` read this session's own 0086 — which also has no such line — unchanged. An invalid value is a loud refusal, not a fail-open (`0216`: *"records close_by: 'sometimes', which is neither"*) | **PASS** |
| AC10 | `002`'s Light row: `$4.20 \| **−26%**` and the mechanism named. `−32%` survives only as a dated correction in `002:59` and `003:78`, never as a live price; `−18%` is labelled *"PROJECTED AND NOT YET MEASURED"* with the 2026-10-31 date. `003` exists (10K) with *Rejected alternatives* and *Residual risk*. M13 restored `$3.89/−32%` to the row → `cost-by-category.test.sh` 1 failed **and** `close-by.test.sh` 3 failed; M14 removed the forecast label → 1 failed | **PASS** |
| AC11 | The level table holds `verify` and `review` as separate rows beside the three pyramid levels, and the prose tells them apart (*"but a mechanical check does"* / *"no mechanical check either"*). This session ran at `unit` against a config with no `review:` block and never consulted one. M7 deleted the `verify` row → 1 failed | **PASS** |
| FR3 both routes | Each proved in **both** directions. Conventional-and-not-executable: `src/thing.test.ts` at mode 100644 closes (`0210`); M4 removing that route → 2 failed. Executable-at-an-unconventional-path: `bin/check-the-thing` closes (`0209`); M3 removing that route → 2 failed. A tracked prose document is refused (`0207`: *"cites docs/notes.md, which is committed but is not an assertion"*) — the hole found on 2026-09-08 is shut. An untracked `tests/` path is refused as *"names no committed path"*. M5 collapsed the two reasons into one string → 2 failed, so they stay told apart | **PASS** |
| NFR Documentation | Within `skills/`, only `skills/queue/templates/close` states what counts as a committed assertion; `queue:281-294` states the *setting* precondition (its own job under FR5) and points at `close` rather than listing the shapes. Two watch-items, neither a breach — see *Findings* | **PASS** |
| NFR Testing | Both sentences now hold. Every close-path row above was driven against a real fixture, not read. **An unperformed checklist is now distinguishable from a performed one:** `0211` refuses where it closed last pass, and the counter is asserted on ticked-ness (`rc_ticked` counts `/^- \[[xX]\][ \t]/` alone), proved by M1 reddening 8 | **PASS** |
| NFR Migration | Proved against an item with no `close_by:` line (`0208`), against both refusal paths for an absent field (`0203`), and against this repo's own ~60 items, none of which carry the field: `./next --drift` reported *"no drift"* before the close | **PASS** |
| Newly reachable (Step 4) | The one new route to `status: done` is a build session closing its own row. Gated on `next: develop` **and** `close_by: develop` **and** every AC citing a path that is conventionally named or committed executable — all three driven above. The gate's remaining weakness is that it cannot tell a real guard tests *this* criterion, and `003`'s *Residual risk* now says so in as many words | **PASS** |
| Always-on (`CONVENTIONS_CORE.md`) | Resolved at `../ai-building-conventions/`. Shell: every input validated before any write, five refusals ahead of the first mutation, each naming the offending value; fails closed where `git` cannot read the repo rather than degrading to a file test; `describe_citation_problem` is one question per function and keeps the loop two deep; `ls-files … -- ":/$tok"` is `--`-terminated. No secrets, no new log field, no analytics event, no egress — the privacy pass does not fire. No auth, credential or visibility change; no UI, so no accessibility surface. Tests shipped with the fix in the same range | **PASS** |
