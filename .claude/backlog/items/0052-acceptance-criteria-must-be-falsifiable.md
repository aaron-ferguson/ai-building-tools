...
| AC6 | `tests/citations.test.sh`. Mutation of the citation this ticket added in `queue`'s step: `` `testing-conventions.md`'s; three shapes `` → `` `absent-conventions.md`'s; three shapes `` | PASS — exit 1, `45 passed, 1 failed`: *"skills/queue/SKILL.md cites \"absent-conventions.md\", which is not a file in …/ai-building-conventions"*. Control `46 passed, 0 failed` |
