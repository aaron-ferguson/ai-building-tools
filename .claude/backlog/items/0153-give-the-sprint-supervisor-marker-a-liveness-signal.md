...
claimed_by: "e2f4"
claimed_at: 2026-09-14T01:57:09Z
touches:
---
...
### Verification 2026-09-13 — session 091e8968, token e2f4 — **FAIL**

**Suite — `config.yml` `commands.unit`, run per file so every file reports, at 5b4e218, every file 0 failed.** Per-file tallies, pasted: `backlog-scripts-installed 37/0 · batching 39/0 · citations 46/0 · claim 98/0 · close-by 67/0 · close 246/0 · cost-by-category 29/0 · cross-cutting-change 21/0 · design-hold 34/0 · external-feedback 9/0 · falsifiable-acs 34/0 · findings-buffer 46/0 · findings-routing 41/0 · floor-probe 12/0 · graph-fields 36/0 · handoff 132/0 · item-ac-form 4/0 · last-line 17/0 · measurement 131/0 · money-in-skill-prose 12/0 · next 432/0 · qa-level-once 11/0 · reference-size 15/0 · release 45/0 · remote-anchor 20/0 · reporting 23/0 · retro-tool-edit 50/0 · skill-size 27/0 · sprint-ledger 106/0 · sprint 239/0/0 skipped · transient-mutation 6/0`.
Copy under test: the repo copy (`skills/sprint/SKILL.md`, `tests/sprint.test.sh`). Dirty set at Step 2 and at the last capture: `?? .claude/backlog/runs/` only. Each mutation was applied to committed `skills/sprint/SKILL.md`, confirmed by a one-line `git diff --stat`, and restored by path. Control run: `239 passed, 0 failed, 0 skipped`. Item written through Bash; the Edit/Write tools were not tried.

| Row | How checked | Result |
|---|---|---|
| AC1 | guard `0153 AC1`; Step 1 (SKILL.md 95–102) ages the `held-by` timestamp against `lock_stale_seconds`, with no pid-alive check. Earlier mutations M1/M-pidalive redden it; not re-run, the guard and passage are unchanged since 7d814a75 | ✅ |
| NFR Documentation | guard `0153 NFR`; the clause *"A pid cannot serve as a liveness signal"* is present at SKILL.md 96 | ✅ |
| AC2 — M-target (write to `.active/held-by.beat`) | → `FAIL 0153 AC2` exact path, `FAIL 0153 AC2` stamp, `FAIL 0153 FR1` loop; **236 passed, 3 failed** | ✅ now reds |
| AC2 — M-notime (`"2026-01-01T00:00:00Z" > …/held-by`) | → `FAIL 0153 AC2 — the held-by write does not stamp $(date -u …)`; **238 passed, 1 failed** | ✅ now reds |
| **AC2 — M-comment** (the write line becomes `# printf '%s %s\n' "$RUN_ID" … > .claude/backlog/runs/.active/held-by`) | → **239 passed, 0 failed** | ❌ the loop no longer writes anything, but the guard greps the text of the line, comment or not |
| **AC2 — M-nocall** (`beat & BEAT=$!` → `: & BEAT=$!`: the heartbeat function is defined and never started) | → **239 passed, 0 failed** | ❌ nothing refreshes `held-by`; the background check matches any `& VAR=$!` |
| Always-on (CONVENTIONS_CORE.md) | a test-only change; no secrets, no egress | ✅ |
