...
- **2026-09-10 (design, from 0060)** — this question is settled by 0060's design pass: the gate counts **every** entry, correct because `retro` Step 4 is now the terminal sweeper (*no entry survives the pass that read it*), and the marker survives as a greppable **head token** — `- 2026-09-05 [->0060] — **what happened.**` — written on the way out by a sweeper, never at write time, and counted like any other entry. That is FR1 and FR7 there, and it answers this row's *marker or two halves* and *should the count skip a marked entry* together. Two halves was rejected: it rewrites an entry at sweep time and lets the halves drift. **This row needs a `queue` decision — withdraw it, or re-scope it to whatever 0060 leaves undone** — rather than a second design pass.
- **2026-09-12 (design) — WITHDRAWN: every FR here is discharged by 0060, re-verified against the checkout,
  not against 0060's own notes.** FR1 and FR2: the head token `[->NNNN]` in
  `skills/queue/templates/FINDINGS.md`, written by `queue` Step 5 on an entry that is both, and
  `retro` Step 4's *"no entry survives the pass that read it"* — so the lesson half is discharged
  by the terminal sweeper removing the entry, and the work half survives as the row the token names.
  FR3: `./next --findings` counts every entry by design, because every entry left is work a retro can
  still do (`count_findings` comment; 0060 AC1, AC7, AC11, all ticked at close). The open question's
  two options were answered as *marker*, and *two halves* rejected, in 0060's Notes. Nothing is left to
  re-scope to, so the row goes and this file stays as the record; id 0080 is burned.
